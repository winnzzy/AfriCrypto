...
export class P2pController {
  ...
  @Get('offers') findOffers(@Query() query: QueryOffersDto) { return this.p2pService.findOffers(query); }
  @Post('offers') createOffer(@CurrentUser('userId') userId: string, @Body() dto: CreateOfferDto) { return this.p2pService.createOffer(userId, dto); }
  @Get('trades') getMyTrades(@CurrentUser('userId') userId: string) { return this.p2pService.getMyTrades(userId); }
  @Post('trade') initiateTrade(@CurrentUser('userId') userId: string, @Body() dto: InitiateTradeDto) { return this.p2pService.initiateTrade(userId, dto); }
